...
// $HeadURL:
// svn://isvn.usc.edu/software/invt/trunk/saliency/src/Devices/Serial.H $
...
class SerialPort {
 public:
  ...
  /* Read funtion:
  Reads a block of bytes from the opened Serial port.
        @params: Number of bytes to be read
        @return: vector with received bytes*/
  ...
  /*! enablePort will do all of the work of actually setting the config
     properties
  ...
  void configure(const char* dev = "", const int speed = 9600,
                 const char* format = "8N1", const bool flowSoft = false,
                 const bool flowHard = false, const int = 0);

  //! Configure the serial port to search for a device
  void configureSearch(const std::string DeviceDescription,
  ...
  /*! The frame returned by the device should have the following format:
      [start byte] [optional number of data bytes] [data...] ... [...data] [end
     byte]
      ...
      @param frameLength optionally specifies the number of bytes in the frame.
     If this is -1,
                         then the second byte of the frame is assumed to contain
     the total number of
                         bytes in the frame.
      @param timeout the number of seconds to wait before giving up (default of
     -1 is no timeout)
      @return the data contained in the frame */
  // TODO: Implement parity checking in the stop byte
  std::vector<unsigned char> readFrame(const unsigned char frameStart,
