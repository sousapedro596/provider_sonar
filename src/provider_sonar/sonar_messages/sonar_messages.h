...
struct mtHeadCommandMsg {
  ...
  std::vector<uint8_t> Construct() {
    ...
    std::vector<uint8_t> msg = {
        ...
        0x02, 0x1D, 0x0D, 0x23, 0x11, 0x99, 0x99, 0x99, 0x05, 0xE1, 0x7A, 0x14,
    ...
    // between 0..210
    msg[43] = static_cast<uint8_t>(gain * 210);
    msg[44] = static_cast<uint8_t>(gain * 210);
    msg[69] = static_cast<uint8_t>(gain * 210);
    msg[70] = static_cast<uint8_t>(gain * 210);
...
struct mtHeadDataMsg {
  ...
  mtHeadDataMsg(SonarMessage const &msg) {
    ...
    range_scale = ((static_cast<uint16_t>(msg.data[20]) |
                    (static_cast<uint16_t>(msg.data[21]) << 8)) & 0x3FFF) / 10;
...
struct mtHeadCommandShortMsg {
  ...
  std::vector<uint8_t> Construct() {
    ...
    // We set the gain by a value between 0..1 but the sonar want a value
    // between 0..210
    msg[18] = static_cast<uint8_t>(gain * 210);
    msg[19] = static_cast<uint8_t>(gain * 210);
